...
CakeResult cake_calculator(int flour, int sugar) {
    // first input is flour then sugar
    CakeResult result;
    // every cake: flour(100) Sugar(50)
    result.cakes = 0;
    result.flour_left = flour;
    result.sugar_left = sugar;

    while (result.flour_left >= 100 && result.sugar_left >= 50) {
        //cake count increment
        result.cakes++;
        result.sugar_left = result.sugar_left - 50;
        result.flour_left = result.flour_left - 100;
    }
